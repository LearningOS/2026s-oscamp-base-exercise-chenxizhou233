...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let handle = thread::spawn(move || numbers.into_iter().map(|x| x * 2).collect::<Vec<i32>>());
    handle.join().unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let h1 = thread::spawn(move || a.into_iter().fold(0, (|acc, curr| acc + curr)));
    let h2 = thread::spawn(move || b.into_iter().fold(0, (|acc, curr| acc + curr)));
    (h1.join().unwrap(), h2.join().unwrap())
}
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    thread::scope(|s| {
        let h1 = s.spawn(|| a.into_iter().fold(0, |acc, curr| acc + curr));
        let h2 = s.spawn(|| b.into_iter().fold(0, |acc, curr| acc + curr));
        (h1.join().unwrap(), h2.join().unwrap())
    })
}
