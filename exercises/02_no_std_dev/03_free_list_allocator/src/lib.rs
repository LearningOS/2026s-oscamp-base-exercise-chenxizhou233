...
unsafe impl GlobalAlloc for FreeListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        while !curr.is_null() {
            let a = (*curr).size >= size;
            let addr = curr as usize;
            let b = (addr & (align - 1)) == 0;
            if a && b {
                if pre.is_null() {
        ...
        loop {
            let curr = self.bump_next.load(Ordering::SeqCst);
            ...
            if self
                .bump_next
                .compare_exchange(curr, end, Ordering::SeqCst, Ordering::SeqCst)
                .is_ok()
            {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        if end < self.heap_end {
            let n_ptr = ptr as *mut FreeBlock;
            (*n_ptr).size = size;
            (*n_ptr).next = self.free_list_head();
            self.set_free_list_head(n_ptr);
        }
    }
}
